...
export default function BottomNav() {
  ...
  return (
    <div className="absolute left-0 right-0 bottom-0 z-30 flex bg-white border-t border-bordersoft px-1 pt-2.5 pb-3">
      {ITEMS.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex-1 flex flex-col items-center gap-[3px] py-1 text-[9px] font-semibold ${active ? 'text-azul-inst' : 'text-muted'}`}
          >
            <svg viewBox="0 0 24 24" width="18" height="18">{item.icon(color)}</svg>
            {item.label}
          </button>
        )
      })}
