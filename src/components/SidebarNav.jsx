...
const ITEMS_PRINCIPALES = [
  ...
  { id: 'dashboard', label: 'Dashboard tributario', icon: (c) => <path d="M4 20V10M10 20V4M16 20v-7M22 20H2" stroke={c} strokeWidth="1.8" strokeLinecap="round" fill="none" /> },
]
...
// Marcador de posición — reemplaza estos 4 nombres por los que confirmes.
const PROXIMAMENTE = [
...
function NavButton({ label, active, onClick, icon, muted, badge }) {
  const color = active ? '#0B3A60' : muted ? '#6B84A0' : '#DCEBFA'
  return (
    <button
      onClick={onClick}
      disabled={muted}
      className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-[13px] font-semibold transition-colors ${
        active ? 'bg-white text-azul-inst' : muted ? 'text-[#6B84A0] cursor-default' : 'text-sky-100 hover:bg-white/10'
      }`}
    >
      {icon && <svg viewBox="0 0 24 24" width="16" height="16" className="flex-shrink-0">{icon(color)}</svg>}
      {!icon && <span className="w-1 h-1 rounded-full bg-current flex-shrink-0 ml-1" />}
      <span className="flex-1 text-left truncate">{label}</span>
      {badge && (
        <span className="text-[8.5px] font-bold px-1.5 py-[2px] rounded bg-white/15 text-sky-100 flex-shrink-0">
          {badge}
        </span>
      )}
    </button>
...
function Section({ title, defaultOpen = true, children }) {
  ...
  return (
    <div className="mb-1">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-3.5 py-2 text-[10px] font-bold uppercase tracking-wide text-sky-300/60"
      >
        {title}
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" className={`transition-transform ${open ? 'rotate-90' : ''}`}>
...
export default function SidebarNav() {
  ...
  function avisarProximamente(nombre) {
    pushLog(`"${nombre}" — módulo en construcción, próximamente disponible.`)
  }
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-3">
        <Section title="Principal">
          {ITEMS_PRINCIPALES.map((item) => (
            <NavButton key={item.id} label={item.label} icon={item.icon} active={screen === item.id} onClick={() => goScreen(item.id)} />
          ))}
        </Section>

        <Section title="Acciones SUNAT" defaultOpen={false}>
          {ACCIONES_SUNAT.map((item) => (
            <NavButton key={item.id} label={item.label} active={screen === item.id} onClick={() => goScreen(item.id)} />
          ))}
        </Section>

        <Section title="Próximamente" defaultOpen={false}>
          {PROXIMAMENTE.map((nombre) => (
            <NavButton key={nombre} label={nombre} muted badge="Pronto" onClick={() => avisarProximamente(nombre)} />
          ))}
        </Section>
